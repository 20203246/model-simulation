...
class MLP:
    def __init__(self):
        # 修正后的隐藏层和输出层，隐藏层带有正则化表示
        self.dense_mlp = tf.keras.layers.Dense(256, activation= tf.nn.relu,kernel_regularizer=tf.keras.regularizers.L1(1e-4))
...
def __call__(self, inputs):
    ...
    # 添加了dropout层
    embedding = tf.keras.layers.Dropout(0.17)(embedding)
